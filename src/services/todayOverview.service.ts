...
type TodayOverviewParams = {
  date?: unknown;
  rawLimit?: unknown;
};

type TodayOverviewAnomaly = {
...
type TodayOverviewData = {
  ...
  summary: {
    ...
    posOperationsCount: number;
    posOperationsAmount: number;
    supplierPurchaseAmount: number;
    companyTopupAmount: number;
    wifiExpenseAmount: number;
    netCashFlow: number;
    doneExchangeCount: number;
    pendingExchangeCount: number;
  };
  completedToday: {
    invoices: { count: number; amount: number };
    payments: { count: number; amount: number };
    posWork: { operations: number; amount: number; employeesActive: number };
    companyBalanceOps: {
      increaseCount: number;
      decreaseCount: number;
      fixCount: number;
    };
    doneExchanges: { count: number };
  };
  pendingNow: {
    pendingExchanges: { count: number; oldestHours: number };
    debtSubscribers: { count: number; totalDebt: number };
    lowBalanceCompanies: { warningCount: number; criticalCount: number };
  };
  anomalies: TodayOverviewAnomaly[];
  highlights: string[];
  recommendedActions: string[];
  supportingData: {
    topEmployeesToday: EmployeeSummary[];
    lowOrOutCompanies: LowOrOutCompany[];
    topDebtors: TopDebtor[];
    recentPendingExchanges: Array<{
      id: string;
      sypAmount: number;
      usdAmount: number;
      details: string;
...
function toSnapshotMap(snapshot: any): RawMap {
  if (!snapshot?.exists?.()) {
    return {};
  }
  ...
  return raw as RawMap;
}

function toDamascusDateKey(date: Date): string {
...
function resolveDateKey(input: unknown): string {
...
function resolveRawLimit(input: unknown): number {
...
function getPreviousDateKeys(dateKey: string, count: number): string[] {
  const keys: string[] = [];
...
function getRecordTimestamp(record: Record<string, unknown>): number | null {
  return (
...
function averageForDateKeys(map: Record<string, number>, keys: string[]): number {
...
function pushAnomaly(
  anomalies: TodayOverviewAnomaly[],
  anomaly: TodayOverviewAnomaly | null,
) {
  if (!anomaly) return;
  anomalies.push(anomaly);
}

export const getTodayOverviewReport = async (
  params: TodayOverviewParams = {},
  ...
  const [
    ...
    invoicesSnap,
    paymentsSnap,
    balanceLogsTodaySnap,
    wifiBalanceSnap,
    pendingExchangeSnap,
    doneExchangeSnap,
    operationsLogsTodaySnap,
    inquiryLogsTodaySnap,
    posProfitTodaySnap,
  ] = await Promise.all([
    get(ref(database, "Subscribers")),
    get(ref(database, "companies")),
    get(ref(database, `dailyTotal/${dateKey}`)),
    get(ref(database, "Invoices")),
    get(ref(database, "Payments")),
    get(ref(database, `balanceLogs/${dateKey}`)),
    get(ref(database, "WifiBalance")),
    get(ref(database, "exchange/pending")),
    get(ref(database, "exchange/done")),
    get(ref(database, `operationsLogs/${dateKey}`)),
    get(ref(database, `astalamatLogs/${dateKey}`)),
    get(ref(database, `profitLogs/posInvoices/${dateKey}`)),
  ]);
  ...
  const invoices = Object.values(invoicesMap) as Array<Record<string, unknown>>;
  const payments = Object.values(paymentsMap) as Array<Record<string, unknown>>;
  const wifiExpenses = Object.values(wifiBalanceMap) as Array<Record<string, unknown>>;
  ...
  const wifiExpensesToday = wifiExpenses.filter(
    (expense) => toDateKey(expense.timestamp) === dateKey,
  );

  const invoicesCount = invoicesToday.length;
  const invoicesAmount = invoicesToday.reduce(
    (sum, invoice) => sum + toNumber(invoice.Amount),
    0,
  );
  const paymentsCount = paymentsToday.length;
  const paymentsAmount = paymentsToday.reduce(
    (sum, payment) => sum + toNumber(payment.Amount),
    0,
  );
  const wifiExpenseAmount = wifiExpensesToday.reduce(
  ...
  Object.entries(dailyTotalMap).forEach(([employeeKey, operationsRaw]) => {
    const operations = Object.values((operationsRaw || {}) as RawMap) as Array<
  ...
  const balanceLogsToday: Array<Record<string, unknown>> = Object.entries(
    balanceLogsTodayMap,
  ).map(([id, logRaw]) => ({
    id,
    ...(logRaw as RawMap),
  ...
  const lowOrOutCompanies: LowOrOutCompany[] = [];

  companies.forEach((company) => {
    if (company.balance <= 0) {
      lowOrOutCompanies.push({
        ...company,
        status: "critical",
      });
      return;
  ...
  const criticalCompanies = lowOrOutCompanies.filter(
    (company) => company.status === "critical",
  );
  const warningCompanies = lowOrOutCompanies.filter(
    (company) => company.status === "warning",
  );
  ...
  const topDebtors: TopDebtor[] = subscribers
  ...
  const debtTotalAmount = topDebtors.reduce(
  ...
  const pendingExchanges = Object.entries(pendingExchangeMap).map(([id, value]) => {
    ...
    const ageHours =
      timestamp === null ? 0 : round2(Math.max(0, (nowMs - timestamp) / 3_600_000));
  ...
  const oldestPendingHours = pendingExchanges.reduce(
    (max, item) => (item.ageHours > max ? item.ageHours : max),
    0,
  );

  pendingExchanges.sort(
    (a, b) => (b.timestamp || 0) - (a.timestamp || 0),
  );

  const doneExchanges: Array<Record<string, unknown>> = Object.entries(
    doneExchangeMap,
  ).map(([id, value]) => ({
    id,
    ...((value || {}) as RawMap),
  ...
  const doneExchangeCount = doneExchanges.filter((exchange) => {
    const timestamp =
      toTimestamp(exchange.timestamp) ||
  ...
  const posProfitTodayLogs = Object.values(posProfitTodayMap) as Array<
    Record<string, unknown>
  >;
  const posProfitTodayCount = posProfitTodayLogs.length;
  const posProfitTodayAmount = posProfitTodayLogs.reduce(
    (sum, record) => sum + toNumber(record.amount),
    0,
  );
  const posProfitTodayProfitAmount = posProfitTodayLogs.reduce(
    (sum, record) => sum + toNumber(record.profitAmount),
    0,
  );
  const failedPosProfitCount = posProfitTodayLogs.filter((record) =>
    isFailedOperationState(record.operationState),
  ).length;

  const netCashFlow = paymentsAmount - (wifiExpenseAmount + supplierPurchaseAmount);

  const invoiceAmountsByDay: Record<string, number> = {};
  const paymentAmountsByDay: Record<string, number> = {};

  invoices.forEach((invoice) => {
    const recordDateKey = toDateKey(invoice.Date);
    if (!recordDateKey) return;
  ...
  const anomalies: TodayOverviewAnomaly[] = [];
  ...
  pushAnomaly(
    ...
    criticalCompanies.length > 0
      ? {
          ...
          suggestedAction:
            "Top up critical supplier balances immediately to prevent service interruption.",
        }
      : null,
  );
  ...
  pushAnomaly(
    ...
    warningCompanies.length > 0
      ? {
          ...
          suggestedAction:
            "Plan proactive top-ups for warning-level companies before they become critical.",
        }
      : null,
  );
  ...
  pushAnomaly(
    anomalies,
    invoicesCount + paymentsCount + posOperationsCount === 0
      ? {
          severity: "medium",
          code: "NO_ACTIVITY_TODAY",
          message:
            "No invoices, payments, or POS operations were recorded for the selected date.",
          evidence: {
            invoicesCount,
            paymentsCount,
            posOperationsCount,
          ...
          suggestedAction:
            "Verify integrations and staff workflow to confirm operational activity is being recorded.",
        }
      : null,
  );
  ...
  pushAnomaly(
    ...
    netCashFlow < 0
      ? {
          ...
          suggestedAction:
            "Review high-cost outflows and prioritize payment collection to restore positive cash flow.",
        }
      : null,
  );

  const hasBacklogAnomaly =
    pendingExchangeCount >= BACKLOG_COUNT_THRESHOLD ||
    oldestPendingHours > BACKLOG_AGE_HOURS_THRESHOLD;
  const isHighBacklog =
    pendingExchangeCount >= HIGH_BACKLOG_COUNT_THRESHOLD ||
  ...
  pushAnomaly(
    ...
    hasBacklogAnomaly
      ? {
          ...
          message:
            "Pending exchange workload is above the healthy backlog threshold.",
          evidence: {
            pendingExchangeCount,
            oldestPendingHours,
          },
          suggestedAction:
            "Assign a focused pass to clear old pending exchanges and reduce queue pressure.",
        }
      : null,
  );
  ...
  pushAnomaly(
    ...
    failedPosProfitCount > 0
      ? {
          ...
          suggestedAction:
            "Inspect failed POS operations and retry or correct the impacted transactions.",
        }
      : null,
  );
  ...
  if (invoiceAverageLast7 > 0) {
    ...
    pushAnomaly(
      anomalies,
      invoiceRatio < DROP_RATIO_THRESHOLD || invoiceRatio > SPIKE_RATIO_THRESHOLD
        ? {
            severity: "medium",
            code: "SIGNIFICANT_DROP_OR_SPIKE",
            message:
              invoiceRatio < DROP_RATIO_THRESHOLD
            ...
            suggestedAction:
              "Validate invoice volume changes against campaign plans, outages, or process issues.",
          }
        : null,
    );
  }
  ...
  if (paymentAverageLast7 > 0) {
    ...
    pushAnomaly(
      anomalies,
      paymentRatio < DROP_RATIO_THRESHOLD || paymentRatio > SPIKE_RATIO_THRESHOLD
        ? {
            severity: "medium",
            code: "SIGNIFICANT_DROP_OR_SPIKE",
            message:
              paymentRatio < DROP_RATIO_THRESHOLD
            ...
            suggestedAction:
              "Validate payments movement against expected collection cycles and follow-up plans.",
          }
        : null,
    );
  }
  ...
  if (invoicesCount > 0) {
    highlights.push(
      `Issued ${invoicesCount} invoices with total value ${round2(invoicesAmount)}.`,
    );
  }
  if (paymentsCount > 0) {
    highlights.push(
      `Collected ${round2(paymentsAmount)} from ${paymentsCount} payments.`,
    );
  }
  if (posOperationsCount > 0) {
    highlights.push(
      `POS team completed ${posOperationsCount} operations across ${employeeSummaries.length} active employees.`,
    );
  }
  ...
  const recommendedActions = Array.from(
    new Set(anomalies.map((anomaly) => anomaly.suggestedAction)),
  );
  ...
  return {
    ...
    supportingData: {
      ...
      lowOrOutCompanies: lowOrOutCompanies.slice(0, rawLimit).map((company) => ({
        ...company,
        balance: round2(company.balance),
        balanceLimit: round2(company.balanceLimit),
      })),
      topDebtors: topDebtors.slice(0, rawLimit).map((subscriber) => ({
        ...subscriber,
        balance: round2(subscriber.balance),
        debtAmount: round2(subscriber.debtAmount),
      })),
